...
var Route = module.exports = function (request, response, routeDef, next) {
    ...
    if (route.routeDef.workingDir) {process.chdir(route.routeDef.workingDir);}
...
Route.prototype.isValid = function (next) {
    ...
    existsCallback = function (exists) {
        if (!exists) {
            return route.next()
        }
    ...
    fs.exists(route.path, existsCallback);
};

Route.prototype.checkVerb = function () {
    var route = this, verbRegEx = /\/new$|\/get$|\/udpate$|\/delete$/i || "";

    if (verbRegEx.test(route.callPath)) {
        route.callPath = route.callPath.replace(verbRegEx,"");
        return verbRegEx.match(route.callPath)[0];
    }
...
Route.prototype.moduleDependency = function (next) {
    ...
    if (!route.module || !route.module[route.action] || !dependency) {
        return next();
    }
...
Route.prototype.load = function (next) {
    ...
    if (!route.module || !route.module[route.action]) {
        return next();
    }
